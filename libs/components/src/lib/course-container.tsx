...
export default function CourseContainer({ course, allCourses }: Props) {
  const response = useSWR('/api/courses', {
    initialData: allCourses,
    refreshInterval: 5000,
  });
  const updatedCourses = response.data || [];
  const updatedCourse =
    updatedCourses.find((s: Course) => s.slug === course.slug) || course;
  ...
  return (
    <div className={styles.container}>
          ...
          !course.isLive ? (
            ...
            >
              <iframe
                ...
                allowFullScreen
                frameBorder="0"
                src={`${updatedCourse.stream}?autoplay=1&mute=1`}
            ...
            <Room
              backstagePeers={course.backstagePeers}
              coursePeers={course.coursePeers}
      ...
      {course.isLive ? (
        ...
        <ScheduleSidebar allCourses={updatedCourses} />
      )}
    </div>
  );
}
