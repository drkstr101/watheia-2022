import { getPageUrl } from '@watheia/cabbage.model';
  ...
  return (
    <Link
      ...
      className={classNames(
        ...
        post.styles?.self?.margin
          ? mapStyles({ margin: post.styles?.self?.margin })
          : undefined,
        ...
        {hasThumbnail && (
          <ImageBlock
            ...
            className={classNames({
              'xs:w-[50%] xs:shrink-0':
                hasBigThumbnail &&
                (flexDirection === 'row' || flexDirection === 'row-reversed'),
...
function mapFlexDirectionStyles(flexDirection: string, hasThumbnail: boolean) {
  switch (flexDirection) {
    ...
    case 'row-reverse':
      return hasThumbnail
        ? 'flex-col xs:flex-row-reverse xs:items-stretch'
        : 'flex-col';
    case 'col':
      return 'flex-col';
    case 'col-reverse':
