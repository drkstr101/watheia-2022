...
export default function FeaturedPeopleSection(props: Props) {
  const {
    elementId,
    colors,
    backgroundImage,
    ...
    >
      <div
        className={classNames(
          ...
          'flex-col',
          mapStyles({
            alignItems: styles?.self?.justifyContent ?? 'flex-start',
          })
        )}
      >
        {badge && (
        ...
        <FeaturedPeopleSectionActions
          actions={actions}
          hasTopMargin={
            !!(badge?.label || title?.text || subtitle || people.length > 0)
          }
        />
      </div>
    </Section>
...
function FeaturedPeopleVariants(props: Partial<Props>) {
...
function FeaturedPeopleThreeCol({
  people = [],
  hasTopMargin,
  hasSectionTitle,
}: Partial<Props>) {
...
function FeaturedPeopleFourCol({
  people = [],
  hasTopMargin,
  hasSectionTitle,
}: Partial<Props>) {
...
function FeaturedPeopleMixedCol({
  people = [],
  hasTopMargin,
  hasSectionTitle,
}: Partial<Props>) {
  ...
  return (
    <div
      ...
      className={classNames(
        ...
        'overflow-hidden',
        styles?.self?.margin
          ? mapStyles({ margin: styles?.self?.margin })
          : undefined,
        styles?.self?.padding
