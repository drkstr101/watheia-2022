...
- Multiple courses - with the ability to add multiple sessions on each course
...
The virtual events starter kit now has added support for organising truly LIVE virtual events. You can quickly setup a live course and invite instructors to interact with viewers. Live audio-video opens up a ton of possibilities with respect to what you can do with this template. You can use it for:

1. Live Webinars
2. Community Calls
3. Hackathons
4. Panel Discussions
5. Multi-course live conferences with a backstage (coming soon)
...
There are four different courses included in the seed data. Feel free to add or remove these based on your schedule. Each course can be easily configured to be a Live Video/Audio experience or an embedded YouTube stream. (You can do all of these configurations via DatoCMS console)

### Joining a course

Visit `/course/a` after entering your email you should see a "Enter your name" input form.

> NOTE: by default, you will join as a Viewer

![join](/media/join.png)

Click on Join and you should see "No Instructors Present". This is because only you have joined the Course aka the "Room" as a viewer. A viewer does not have the permission to publish their audio and video. You can read more about roles in the sections below.

![course.png](/media/course.png)

### Joining with different Roles

For this we pass a query param in url for eg: `/course/a?role=<ROLE_NAME>`

- Admin: `/course/a?role=backstage`

- Instructor: `/course/a?role=stage`

- Student: `/course/a`

So if you visit `/course/a?role=stage` now you should see a Preview screen opening up. After joining you should be able to see yourselves. Open a new tab or invite others to host your next meetup, community calls, etc.
...
Once it’s set up, sign up using the registration form on the home page (not on a course page) and then click "Generate with GitHub".
